    ...
    var edge = {/*
        ...
        draw : function() {
            ...
            var x1 = p[res[0]].x,
                ...
                x4 = p[res[1]].x,
                y4 = p[res[1]].y;
            if (x4 - x1 >= 0 && style && style.directed) {
                y1 = y1 + 15;
                y4 = y4 + 15;
            } else if (style && style.directed) {
                y1 = y1 - 15;
                y4 = y4 - 15;
            }
            ...
            edge.bg && edge.bg[move]({path:path})
                || style && style.fill && (edge.bg = style.fill.split && selfRef.path(path).attr({stroke: style.fill.split("|")[0], fill: "none", "stroke-width": style.fill.split("|")[1] || 8}).toBack());
            /* setting label */
            style && style.label 
                && (edge.label && edge.label.attr({x:(x1+x4)/2, y:(y1+y4)/2}) 
                    || (edge.label = selfRef.text((x1 + x4) / 2, (y1 + y4) / 2, style.label).attr({ fill: style["label-style"]["color"] || "#000", "font-size": style["font-size"] || "12px" })));
